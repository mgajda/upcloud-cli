...
Define the credentials using `UPCLOUD_TOKEN` environment variable (preferred) or by setting `UPCLOUD_USERNAME` and `UPCLOUD_PASSWORD` environmental variables,
or in the `upctl` config file (default `~/.config/upctl.yaml`, overridable with `--config /path/to/upctl.yaml`):

```yaml
# Preferred: API token (unaffected by 2FA, recommended for automation)
token: "your-token"

# Legacy method: username and password (affected by 2FA - will not work if 2FA is enabled)
